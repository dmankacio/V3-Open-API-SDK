...
instrumentId = "BCH-USD-SWAP"
granularity = 1800
# channels = ["swap/account:BCH-USD-SWAP", "swap/position:BCH-USD-SWAP", "swap/order:BCH-USD-SWAP", "swap/candle300s:BCH-USD-SWAP"]
channels = [f"swap/account:{instrumentId}", f"swap/position:{instrumentId}", f"swap/order:{instrumentId}", f"swap/candle{granularity}s:{instrumentId}"]
    ...
    if clientId is not None:
        ...
        if curPair is not None:
            ...
            if revokeOid is not None and revokeOid != '-1':
                result2 = swapAPI.revoke_order(revokeOid, granularity)
...
def klineMatch():
    ...
    curAvg7 = getAvgX(candle300s, -8, -1) #7均线
    curAvg34 = getAvgX(candle300s, -35, -1) #34均线
    preAvg7 = getAvgX(candle300s, -9, -2) #前个7均线
    preAvg34 = getAvgX(candle300s, -36, -2) #前个34均线

    if buyOrSell == 'buy' and preAvg7 < preAvg34 and curAvg7 > curAvg34:
        #金叉买入
        print(f'will buy, preAvg7:{preAvg7}<preAvg34:{preAvg34}, {curAvg7}(curAvg7)>{curAvg34}(curAvg34)')
        takeOrderMatch(instrumentId)
    elif buyOrSell == 'sell' and preAvg7 > preAvg34 and curAvg7 < curAvg34:
        #死叉卖出
        print(f'will sell, preAvg7:{preAvg7} > preAvg34:{preAvg34}, {curAvg7}(curAvg7) < {curAvg34}(curAvg34)')    
        takeOrderMatch(instrumentId)
    else:
        print(f'buyOrSell: {buyOrSell}, preAvg7:{preAvg7}, preAvg34:{preAvg34}, curAvg7={curAvg7}, curAvg34={curAvg34}  --- {buyOrSell}')
...
def on_message(ws, message):
    ...
    if 'event' in resDict:
        ...
        if resTbl == f'swap/{granularity}': #最新价
...
def on_open(ws):
    ...
    klines = swapAPI.get_kline(instrumentId, granularity, beginT, endT)
