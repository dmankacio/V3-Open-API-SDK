    ...
    if marginRatio > marginRatioGate:
        ...
        if int(curPosition) > amount:
            ...
            for r in result['order_info']:
                print(f"take_orders.{typeUp}.order: {r['client_oid']}<->{r['order_id']}")
        else:
            ...
            for order in resData:
                ...
                if order['state'] == '0':
                    ...
                    if myAccount is not None:
                        takeOrder(order['instrument_id'], thisPosition, curPrice, inPrice, outPrice, odrTypeA, odrTypeB)
                elif order['state'] == '-1':
                    #撤单成功
                    print(f"revoke order successfull clientOid:{order['client_oid']}, orderId:{order['order_id']}")
                    #删除记录对
                    if 'client_oid' in order:
